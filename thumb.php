...
if(!empty($_GET['t'])) {
	if($_GET['t'] == 'l') {
		...
		$quality_list = array(50, 75, 100, 479);
	} elseif($_GET['t'] == 'm') {
		$type = 'uploads/media';
...
$_GET['src'] = "http://localhost".'/'.$type.'/'.$_GET['src']; 
...
if(! defined('FILE_CACHE_DIRECTORY') ) 		define ('FILE_CACHE_DIRECTORY', './storage/cache/images');				// Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
